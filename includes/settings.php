...
function koalum_add_admin_menu() {
    ...
    add_submenu_page('koalum', 'Historial de Reservas', 'Historial de Reservas', 'manage_options', 'koalum_historial', 'koalum_historial_page');
    add_submenu_page('koalum', 'Personalización de Emails', 'Emails', 'manage_options', 'koalum_emails', 'koalum_emails_page');
    add_submenu_page('koalum', 'Configuración de Horarios', 'Horarios', 'manage_options', 'koalum_horarios', 'koalum_horarios_page');
}
add_action('admin_menu', 'koalum_add_admin_menu');

// Función para mostrar el historial de reservas
function koalum_historial_page() {
    global $wpdb;
    ...
    if (isset($_GET['email'])) {
        ...
        if ($reservas) {
            ...
            foreach ($reservas as $reserva) {
                $reserva_id = isset($reserva->id) ? esc_html($reserva->id) : '';  // Cambio aquí
